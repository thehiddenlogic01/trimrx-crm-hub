...
## Recent Changes
- 2026-03-11: Added Audit Report page (`/admin/audit-report`) under Admin section — tracks all user actions across CV Report, Manage Slack Case, and Retention Final Submit pages. Logs: Reply Sent, Mark as Done, Unmark Done, Send to CV Report (CV Report Created), CV Report Updated, CV Report Deleted, Delete Message, Push to Google Sheets. Database table: `audit_logs` (id, user_id, username, action, page, details, created_at). Backend: `server/audit-logs.ts` with `logAudit()` helper function and `GET /api/audit-logs` endpoint. Frontend: filterable by user, page, action, and date range; paginated (25/50/100 per page). Admin-only access.
- 2026-03-10: Added "Slack Backlog All" page (`/trimrx/slack-backlog-all`) under TrimRX CV — independent duplicate of "Manage Slack Case" with its own page component (`client/src/pages/trimrx/slack-backlog-all.tsx`), separate permission page key (`slack-backlog-all`), and separate feature permissions. Changes to this page do not affect the original "Manage Slack Case" and vice versa. Registered in sidebar, routing, sections, features, and admin settings.
- 2026-03-09: Added Sidebar Section Order and Menu Item Order controls in Admin Settings — admin can reorder sections (up/down arrows) and reorder menu items within each section. Admin section is pinned at bottom. Uses `sidebar_section_order` (array of section labels) and `sidebar_menu_order` (object mapping section labels to route arrays) keys in appSettings. Backend validates array vs object types per key and enforces admin-only write access on sidebar settings.
- 2026-03-08: Added Sidebar Menu Visibility control in Admin Settings — admin can toggle show/hide for any sidebar menu item (except Admin section). Uses `sidebar_hidden_items` key in appSettings. Sidebar fetches hidden items and filters them from navigation. All menu items appear automatically grouped by section with Eye/EyeOff icons and toggle switches.
- 2026-03-08: Added CV Settings page (`/trimrx/cv-settings`) under TrimRX CV — manage dropdown options for CV Report and Retention Final Submit tables. Starting with "Slack Status (RT)" dropdown. Options stored in `app_settings` table (key: `slack_status_rt_options`). Backend: `server/cv-settings.ts` with GET/POST `/api/cv-settings/:key`. Both cv-report.tsx and retention-final-submit.tsx now fetch options dynamically from settings API with hardcoded fallback. Extensible for more dropdown sections.
